...
<h1 style="margin-left: 5px;">Giriş Yap</h1>';
		$simge=guvenlik($_POST['oadi']);
		$sifre=guvenlik($_POST['osifre']);
		
		$simge = htmlspecialchars($simge);
		$sifre = htmlspecialchars($sifre);
				
		$sorgu = @mysql_query("SELECT * FROM ogrenci WHERE ogrNo='".$simge."' AND ogrSifre ='".md5($sifre)."'");
  ...
  <a href="javascript:history.back()"><big><b>Geri Dön</b></big></a>
   ...
   elseif (@mysql_num_rows($sorgu) == 1) {
						...
						setcookie("ogrOturum", $oturum, $zaman);
						setcookie("ogrKim", $satir['ogrKim'], $zaman);
												
						mysql_query("UPDATE ogrenci SET ogrOturum = '$oturum'
						WHERE ogrKim = '".$satir['ogrKim']."'");
